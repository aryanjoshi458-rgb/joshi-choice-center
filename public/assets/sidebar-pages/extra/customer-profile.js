...
async function initProfile() {
    ...
    if (!customer) {
        // If data is synced but still not found, then it's a real error
    ...
    const parseSafeDate = (dateStr) => {
        ...
        if (isNaN(d)) {
            ...
            if (parts.length === 3) {
                const isoDate = parts[0].length === 2 ? `${parts[2]}-${parts[1]}-${parts[0]}` : datePart;
    ...
    sortedTxns.forEach(t => {
        ...
        let label3 = sName.includes("withdrawal") ? "Cash to Customer" : "Total Received";
        let val3 = sName.includes("withdrawal") ? (amt - charge) : (amt + charge);
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (window.auraSyncComplete) initProfile();
